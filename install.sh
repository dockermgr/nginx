...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202308261948-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  WTFPL
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2023 Jason Hempstead, Casjays Developments
# @@Created          :  Saturday, Aug 26, 2023 19:48 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for nginx
...
__printf_spacing_color() { printf "%b%-${2:?Please set spacing number}s%s%b\n" "$(tput setaf "${1:?Please set color number}" 2>/dev/null)" "${3:?Please set left content}" "${4:?Please set right content}" "$(tput sgr0 2>/dev/null)"; }
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
HUB_IMAGE_URL="casjaysdevdocker/nginx"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [nginx.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set custom script - saves to /config/env/nginx.script.sh
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-nginx}}"
...
if [ -n "$NGINX_USERNAME" ]; then
  CONTAINER_USER_NAME="$NGINX_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${NGINX_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
